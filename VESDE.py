...
class VESDE(nn.Module):
    def __init__(self, sigma_min, sigma_max, eps):
        super().__init__()
    ...
    def get_sigma_t(self, t):
        return torch.tensor(self.sigma_min*((self.sigma_max/self.sigma_min) **t)).to(DEVICE)
    ...
    def sde(self, x, t):
        ...
        diffusion = self.get_sigma_t(t) * torch.sqrt(2*torch.log(torch.tensor(self.sigma_max/self.sigma_min))).to(DEVICE).view(-1, 1, 1, 1)
...
sbgm = VESDE(sigma_min=0.01, sigma_max=50, eps=1e-3).to(DEVICE)
